...
export function buildInvoiceHtml({
  ...
  // VAT applies to this period's supply (the earned amount) — never to carried
  // credit, which is a settlement/financing adjustment, not a taxable supply.
  const vat = vatBreakdown(a.earnedCents, invoice.vat_rate);
  const showVat = vat.rate > 0;

  // "Billed this period": the tax invoice for the work done this month.
  ...
  if (showVat) {
    billedRows.push(
      { label: 'Earned (net)', value: money(vat.netCents) },
      { label: `VAT (${vat.rate}%)`, value: money(vat.taxCents) },
      { label: 'Total (incl. VAT)', value: money(vat.grossCents), strong: true },
    );
  } else {
    billedRows.push({ label: 'Earned', value: money(a.earnedCents), strong: true });
  }

  // "Settlement (net)": the running funding ledger — all figures are net of VAT,
  // matching the funding engine (which allocates net earned amounts).
  const settlementRows: { label: string; value: string; strong?: boolean }[] = [
    { label: 'Earned (net)', value: money(a.earnedCents) },
    { label: 'Credit brought forward', value: money(a.creditBroughtForwardCents) },
    { label: 'Amount due (net)', value: money(a.amountDueCents), strong: true },
    { label: 'Amount paid', value: money(a.amountPaidCents) },
    { label: 'Carried forward', value: money(a.creditCarriedForwardCents) },
    { label: 'Outstanding (net)', value: money(a.outstandingCents), strong: true },
  ];
...
<body>
  <div class="doc">
    ...
    <div class="section-label">Billed this period</div>
    <table>${toRows(billedRows)}</table>
    <div class="section-label">Settlement (net)</div>
