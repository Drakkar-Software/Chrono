-- Canonical remuneration rules:
--   • company defaults: fee 5%, rem max 75%, license 30%, hours/day 8, vacation 15
--   • fixed monthly capacity 22 × hours_per_day
--   • rem_license_recipient (exactly two when distributing license)
--   • largest-remainder partner splits
--   • residual OctoBot weighting: max(0, 22 − staffing_days)
--   • global company fee on product_pool, product_service, staffing, jungle
--   • jungle FIFO ordered by (seq, id); dequeue post-fee revenue
--   • product_pool settlement only via designated product_pool_project_id
--   • server-side vacation + monthly capacity enforcement
--   • rem_lines / fee reserve / jungle settlements manager-write revoked (RPC only)

-- ---------------------------------------------------------------------------
-- Schema defaults + license recipient flag
-- ---------------------------------------------------------------------------

alter table public.companies
  alter column company_fee_pct set default 5,
  alter column rem_max_percent set default 75,
  alter column default_license_pct set default 30;
...
with ranked as (
  select
    id,
    company_id,
    row_number() over (partition by company_id order by created_at, user_id) as rn,
    count(*) over (partition by company_id) as n
  from public.company_members
  where deleted = false and rem_partner = true
)
...
where cm.id = r.id
  ...
  and not exists (
    ...
    where x.company_id = r.company_id
      and x.deleted = false
      and x.rem_license_recipient = true
  );

-- ---------------------------------------------------------------------------
-- Largest-remainder split helper (mirrors SDK splitCentsByShares)
-- ---------------------------------------------------------------------------

create or replace function public.rem_split_by_shares(
  p_total bigint,
  p_shares jsonb -- [{user_id: uuid, share: number}, ...]
)
returns table (user_id uuid, amount_cents bigint)
language plpgsql
immutable
as $$
declare
...
begin
  ...
  if p_total is null or p_total = 0 then
    for v_i in 0 .. v_n - 1 loop
      user_id := (p_shares->v_i->>'user_id')::uuid;
      amount_cents := 0;
      return next;
    end loop;
    return;
  ...
  for v_i in 0 .. v_n - 1 loop
    v_uid := (p_shares->v_i->>'user_id')::uuid;
    v_share := coalesce((p_shares->v_i->>'share')::numeric, 0);
    if v_share < 0 or v_share <> v_share then
      v_share := 0;
    end if;
    v_uids := v_uids || v_uid;
    v_weights := v_weights || v_share;
    v_weight_total := v_weight_total + v_share;
  end loop;

  if v_weight_total <= 0 then
    for v_i in 1 .. v_n loop
      v_weights[v_i] := 1.0 / v_n;
    end loop;
    v_weight_total := 1;
  end if;

  v_floors := array[]::bigint[];
  v_rems := array[]::numeric[];
  for v_i in 1 .. v_n loop
    v_exact := v_abs * (v_weights[v_i] / v_weight_total);
    v_floor := trunc(v_exact)::bigint;
    v_floors := v_floors || v_floor;
    v_rems := v_rems || (v_exact - v_floor);
    v_allocated := v_allocated + v_floor;
  end loop;

  v_left := (v_abs - v_allocated)::integer;
  v_order := array[]::integer[];
  for v_i in 1 .. v_n loop
    v_order := v_order || v_i;
  end loop;

  -- Sort indices by remainder desc, then user_id asc
  for a in 1 .. v_n loop
    for b in a + 1 .. v_n loop
      if v_rems[v_order[b]] > v_rems[v_order[a]]
         or (v_rems[v_order[b]] = v_rems[v_order[a]] and v_uids[v_order[b]] < v_uids[v_order[a]]) then
        tmp_i := v_order[a];
        v_order[a] := v_order[b];
        v_order[b] := tmp_i;
      end if;
  ...
  for v_i in 1 .. v_left loop
    v_floors[v_order[((v_i - 1) % v_n) + 1]] := v_floors[v_order[((v_i - 1) % v_n) + 1]] + 1;
  end loop;

  for v_i in 1 .. v_n loop
    user_id := v_uids[v_i];
    amount_cents := v_floors[v_i] * v_sign;
    return next;
  end loop;
end;
$$;

revoke all on function public.rem_split_by_shares(bigint, jsonb) from public;
grant execute on function public.rem_split_by_shares(bigint, jsonb) to authenticated;

-- ---------------------------------------------------------------------------
-- Capacity / vacation / weekday enforcement
-- ---------------------------------------------------------------------------

create or replace function public.company_hours_per_day(p_company_id uuid)
returns numeric
language sql
stable
security definer
...
begin
  ...
  if coalesce(cardinality(v_days), 0) = 0 then
    raise exception 'working_weekdays must not be empty';
  end if;
  foreach v_d in array v_days loop
    if v_d < 1 or v_d > 7 then
      raise exception 'working_weekdays values must be in 1..7';
    end if;
  end loop;
  if (select count(distinct x) from unnest(v_days) x) <> cardinality(v_days) then
    raise exception 'working_weekdays must not contain duplicates';
  end if;
  return new;
end;
$$;

drop trigger if exists enforce_company_working_weekdays on public.companies;
create trigger enforce_company_working_weekdays
  before insert or update of working_weekdays on public.companies
  for each row execute function public.enforce_working_weekdays_shape();

drop trigger if exists enforce_member_working_weekdays on public.company_members;
create trigger enforce_member_working_weekdays
  before insert or update of working_weekdays on public.company_members
  for each row execute function public.enforce_working_weekdays_shape();

-- Validate designated product_pool_project_id
...
begin
  ...
  if not found or v_deleted or v_company is distinct from new.id or v_policy is distinct from 'product_pool' then
    raise exception 'product_pool_project_id must reference an active product_pool project in this company';
  end if;
  return new;
end;
$$;

drop trigger if exists enforce_product_pool_project_trigger on public.companies;
create trigger enforce_product_pool_project_trigger
  before insert or update of product_pool_project_id on public.companies
  for each row execute function public.enforce_product_pool_project();

-- ---------------------------------------------------------------------------
-- Restrict direct ledger writes (RPC / SECURITY DEFINER only)
-- ---------------------------------------------------------------------------

drop policy if exists "managers write rem lines" on public.rem_lines;
drop policy if exists "managers write fee reserve" on public.company_fee_reserve_ledger;

drop policy if exists "managers write jungle queue" on public.jungle_tjm_queue_entries;
drop policy if exists "managers write jungle settlements" on public.jungle_tjm_queue_settlements;

-- Keep read policies; writes go through compute_rem_month / settle RPCs (definer).

-- ---------------------------------------------------------------------------
-- compute_rem_month (canonical)
-- ---------------------------------------------------------------------------
